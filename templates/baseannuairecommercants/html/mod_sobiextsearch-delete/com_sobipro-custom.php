...
$searchword = '<input type="text" name="sp_search_for" size="' . $width
		...
		. '\') this.value=\'\';" class="XTSPSearchBox search-query' . $moduleclass_sfx
...
if ($button)
{
	...
	else
	{
		$button = '<input type="submit" id="XTtop_button" name="search" value="' . $button_text
				. '" class="button' . $moduleclass_sfx . '" onclick="this.form.sp_search_for.focus();extSearchHelper' . $moduleid . '.extractFormValues();"/>'.$button_text.'</input>';
	}
}
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" 
	  id="<?php echo $searchformid; ?>" class="form-search" >
    <div class="XTsearch<?php echo $moduleclass_sfx; ?> input-append">
		<?php
		echo $output;
